...
class Run(object):
    def __init__(self, env, agent):
        ...
        self.line2, = self.ax2.plot(
            range(len(self.episode_reward)), self.episode_reward
        )

    def update_display_step(self):
        if not hasattr(self, "imgplot"):
            self.imgplot = self.ax.imshow(
                self.env.render(mode="rgb_array"), interpolation="none", cmap="viridis"
            )
        else:
    ...
    def run_qlearning(
        self, max_number_of_episodes=100, interactive=False, display_frequency=1
    ):
